...
export default class Board extends React.Component {
    ...
    render() {
        return <div className="flex-container">
            <div className="board">
                <div className="end-turn" style={{ "position": "absolute"}}>
                    <button onClick={this.startGame}>Start Game</button>
                </div>
                <div className="resource row">
                    {this.state.p1 && this.state.p2.deck.map(cardNumber => <Card cardNumber={cardNumber} gameCards={this.state.gameCards}/>)}
                </div>
                ...
                <div className="front row">
                    {this.state.p1 && this.state.p1.hand.map(cardNumber => <Card cardNumber={cardNumber} gameCards={this.state.gameCards} />)}
                </div>
                <div className="back row">
                    {this.state.p1 && [].map(cardNumber => <Card cardNumber={cardNumber} gameCards={this.state.gameCards} />)}
                </div>
                <div className="resource row">
                    {this.state.p1 && this.state.p1.deck.map(cardNumber => <Card cardNumber={cardNumber} gameCards={this.state.gameCards} />)}
                </div>
            </div>
            {(this.state.turn) && <GameState
